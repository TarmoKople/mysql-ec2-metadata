...
static int ec2_meta_fill_table(THD *thd, TABLE_LIST *tables, Item *cond) {
    ...
    if (!ec2_meta_downloaded) {
        my_plugin_log_message(&plugin_info_ptr, MY_INFORMATION_LEVEL, "downloading ec2 meta info...");
        for (uint i = 0; i < sizeof(ec2_meta_keys) / sizeof(*ec2_meta_keys); i++) {
            try {
                http::Request request{ec2_meta_url + ec2_meta_keys[i], http::InternetProtocol::V4};
                ...
                if (response.status.code == http::Status::Ok) {
                    ...
                    my_plugin_log_message(&plugin_info_ptr, MY_INFORMATION_LEVEL, "got info %s", ec2_meta_keys[i].c_str());
                } else {
                    continue;
                }
            }
            ...
            {
                my_plugin_log_message(&plugin_info_ptr, MY_ERROR_LEVEL, "request error: %s", e.what());
            }
            catch (const http::ResponseError& e)
            {
                my_plugin_log_message(&plugin_info_ptr, MY_ERROR_LEVEL, "response error: %s", e.what());

            }
            catch (const std::exception& e)
            {
                my_plugin_log_message(&plugin_info_ptr, MY_ERROR_LEVEL, "error: %s", e.what());
            }
        }
    ...
    std::vector<meta_row_t>::iterator iter;
        for(iter = ec2_meta_rows.begin(); iter != ec2_meta_rows.end(); ++iter){
            table->field[0]->store(iter->key.c_str(), iter->key.length(), system_charset_info);
            table->field[1]->store(iter->value.c_str(), iter->value.length(), system_charset_info);
